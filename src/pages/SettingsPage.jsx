// ─────────────────────────────────────────────────────────────────────────────
// SettingsPage.jsx  v8.1 — multi-section with sidebar navigation
// ─────────────────────────────────────────────────────────────────────────────
...
const _STATIC_T = {
  pageBg:"#07090f",cardBg:"#0d1120",elevated:"#171e30",panelBg:"#0f1422",
...
// Dynamic token getter — light mode aware
function getT() {
  const theme = localStorage.getItem("rs_theme") || "dark";
...
const _Ic=({s=14,color="currentColor",ch})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" style={{display:"block",flexShrink:0}}>{ch}</svg>;
const SIcon={
  ...
  blackout: (p)=><_Ic {...p} ch={<><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18M9 15l6 0M9 19l4 0"/></>}/>,
...
function FRow({label,hint,children}){
  const T = getT();
  return <div style={{display:"flex",alignItems:"flex-start",gap:16,marginBottom:16,flexWrap:"wrap"}}>
...
function STitle({children,sub}){
  const T = getT();
  return <div style={{marginBottom:20,paddingBottom:10,borderBottom:`1px solid ${T.border}`}}>
...
function Note({children,color="elevated"}){
  ...
  return <div style={{padding:"10px 14px",background:bg,border:`1px solid ${bd}`,borderRadius:T.r10,
...
function SBusiness({s,set}){
  ...
  return <>
    ...
    <FRow label="Phone number"><Inp value={s.phone} onChange={e=>set("phone",e.target.value)} placeholder="+1 (416) 555-0000"/></FRow>
    <FRow label="Address"><Inp value={s.address} onChange={e=>set("address",e.target.value)} placeholder="123 Main St, Toronto, ON"/></FRow>
...
function SHours({s,set}){
  ...
  return <>
    <STitle sub="Changes here immediately affect the Shopify booking form — customers only see slots within these hours.">Business hours</STitle>
    <div style={{background:T.elevated,border:`1px solid ${T.border}`,borderRadius:T.r12,overflow:"hidden",marginBottom:12}}>
      {DAYS.map((day,i)=>{
        ...
        return <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 18px",
...
function SBlackout({s,set}){
  ...
  return <>
    ...
    {dates.length===0&&<div style={{fontSize:12,color:T.textMuted,padding:"12px 0",marginBottom:8}}>No blackout dates. Add a date below to block a specific day.</div>}
...
function SServices({s,set}){
  ...
  return <>
    <STitle sub="Duration, bay type, and active state all update the Shopify booking form immediately after saving.">Services</STitle>
    <Note color="blue">Changing service duration or pool type here automatically updates what customers see on the booking form — no code changes needed.</Note>
        ...
        <div key={i} style={{background:T.elevated,border:`1px solid ${svc.active!==false?T.borderVis:T.border}`,borderRadius:T.r12,padding:"16px"}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",marginBottom:12}}>
            <Inp value={svc.name} onChange={e=>{const sv=[...svcs];sv[i]={...sv[i],name:e.target.value};set("services",sv);}}
              placeholder="Service name" style={{flex:"1 1 160px",minWidth:140,fontSize:14,fontWeight:600}}/>
            ...
            <button onClick={()=>set("services",svcs.filter((_,j)=>j!==i))}
              style={{background:T.redBg,border:`1px solid ${T.redBorder}`,borderRadius:T.r8,
                padding:"6px 11px",color:T.redText,fontSize:11,fontFamily:T.font,cursor:"pointer"}}>Remove</button>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10}}>
            ...
            <div>
              ...
              <Sel value={svc.resourcePool||"none"}
                ...
                options={[
                  {value:"bay",label:"Bay (normal)"},
                  {value:"alignment",label:"Alignment lane"},
                  {value:"none",label:"No bay required"},
                ]}/>
            </div>
          </div>
        </div>
...
function SCapacity({s,set}){
  ...
  return <>
    <STitle sub="Controls how many cars can be booked simultaneously. Changes are live immediately on the booking form.">Capacity</STitle>
    <Note color="amber">If you add or remove a bay, update the count here. The booking form will immediately allow or restrict bookings — no code changes needed.</Note>
    <FRow label="Normal bay count" hint="Confirmed bookings consume one bay slot each.">
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{fontSize:12,color:T.textMuted}}>currently {s.bayCount||3} bay{(s.bayCount||3)!==1?"s":""}</div>
      </div>
    </FRow>
    <FRow label="Alignment lane" hint="Completely separate from normal bays — never affects bay capacity.">
    ...
    {s.alignmentLaneEnabled!==false&&(
      <FRow label="Alignment capacity" hint="Usually 1 — how many cars can be in the alignment lane at once.">
    ...
    <div style={{padding:"10px 14px",background:T.elevated,border:`1px solid ${T.border}`,borderRadius:T.r10,fontSize:12,color:T.textSecond,lineHeight:1.7}}>
      Current setup: <strong style={{color:T.textPrimary}}>{s.bayCount||3} normal bay{(s.bayCount||3)!==1?"s":""}</strong>
      {s.alignmentLaneEnabled!==false&&<> + <strong style={{color:T.amberText}}>1 alignment lane</strong> (independent)</>}.
      Tire Purchase and Other consume no bay capacity.
    </div>
  </>;
}
function SSMS({s,set}){
  const TMPL=[
    {key:"confirmed",label:"Appointment confirmed",hint:"Sent when staff confirms a booking"},
    ...
    {key:"completed_review",label:"Completed + review link",hint:"Must end with \\n{reviewLink}"},
  ...
  return <>
    <STitle sub="Leave any template blank to use the system default. Variables are replaced with real values when sent.">SMS templates</STitle>
    <div style={{padding:"10px 14px",background:T.elevated,border:`1px solid ${T.border}`,borderRadius:T.r10,marginBottom:20,fontSize:12,color:T.textSecond,lineHeight:1.8}}>
      ...
      {["{firstName}","{shopName}","{date}","{time}","{service}","{reviewLink}"].map(v=>(
        <code key={v} style={{display:"inline-block",background:T.pageBg,border:`1px solid ${T.border}`,
          padding:"1px 7px",borderRadius:4,marginRight:6,marginBottom:4,color:"#60A5FA",fontFamily:"monospace",fontSize:11}}>{v}</code>
      ))}
    </div>
...
function SReview({s,set}){
  ...
  return <>
    <STitle sub="Paste your Google Business review link. It replaces {reviewLink} in the completion SMS.">Google review link</STitle>
    ...
    {s.googleReviewLink&&<div style={{paddingLeft:196,fontSize:11,color:"#5eead4",marginTop:-8,marginBottom:16}}>
      <a href={s.googleReviewLink} target="_blank" rel="noopener noreferrer" style={{color:"#5eead4"}}>{s.googleReviewLink}</a>
    </div>}
    <Note>To get your Google review link: go to your Google Business Profile → click "Get more reviews" → copy the link shown.</Note>
  </>;
}
  ...
  return <>
    ...
    {s.reminderEnabled!==false&&<>
      <FRow label="Minutes before" hint="Default: 30 minutes before the appointment">
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <Inp type="number" value={s.reminderMinutes||30} onChange={e=>set("reminderMinutes",Math.max(5,Number(e.target.value)))} style={{width:90}}/>
          <span style={{fontSize:12,color:T.textMuted}}>minutes before</span>
        </div>
      </FRow>
      <Note>Reminder template text is set in SMS Templates. Each reminder is sent once — failures are logged in the SMS history on the booking card.</Note>
    </>}
  </>;
}
function SBranding({s,set}){
  ...
  return <>
    ...
    <FRow label="Logo URL" hint="Displayed at the top of the booking form. PNG or SVG recommended.">
      <Inp value={s.logoUrl} onChange={e=>set("logoUrl",e.target.value)} placeholder="https://your-shop.com/logo.png"/>
      {s.logoUrl&&<div style={{marginTop:8,display:"flex",alignItems:"center",gap:10}}>
    ...
    <FRow label="Primary colour" hint="Button and selection accent colour on the booking form.">
...
export default function SettingsPage({onAlert}){
  ...
  const save=async()=>{
    ...
    try{
      ...
      onAlert?.("Settings saved. Changes are live immediately.","info");
      setTimeout(()=>setSaved(false),3000);
    }catch(e){setError(e.message);onAlert?.(e.message,"error");}
    finally{setSaving(false);}
  };

  if(loading) return <div style={{padding:"40px 0",textAlign:"center",color:T.textMuted,fontSize:13}}>Loading settings…</div>;
  if(!settings) return <div style={{color:"#FCA5A5",fontSize:13}}>{error||"Could not load settings."}</div>;

  const p={s:settings,set};
  const MAP={
    business:<SBusiness {...p}/>, hours:<SHours {...p}/>, blackout:<SBlackout {...p}/>,
    services:<SServices {...p}/>, capacity:<SCapacity {...p}/>, sms:<SSMS {...p}/>,
    review:<SReview {...p}/>, reminders:<SReminders {...p}/>, branding:<SBranding {...p}/>,
  };

  return <div style={{display:"flex",gap:0,minHeight:500,alignItems:"flex-start"}}>
    {/* Sidebar */}
    <div style={{width:196,flexShrink:0,marginRight:28,position:"sticky",top:80}}>
      <div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r12,overflow:"hidden"}}>
        {SECTIONS.map(({id,label,icon})=>{
          const a=section===id;
          return <button key={id} onClick={()=>setSection(id)}
      ...
      {dirty&&<div style={{marginTop:10,padding:"8px 12px",background:T.amberBg,border:`1px solid ${T.amberBorder}`,
      ...
      <div style={{position:"sticky",bottom:0,background:T.panelBg,borderTop:`1px solid ${T.border}`,
        ...
        <button onClick={save} disabled={saving}
          style={{padding:"10px 28px",borderRadius:T.r8,background:saved?T.greenBg:T.blue,
            ...
            fontSize:13,fontWeight:600,fontFamily:T.font,cursor:saving?"not-allowed":"pointer",opacity:saving?0.6:1,transition:"all .2s"}}>
