// ─────────────────────────────────────────────────────────────────────────────
// RoadstarDashboard.jsx  v6
// New: Live at Bay section · Google Reviews section · mobile hamburger header
//      Fixed completed_review SMS · new services · capacity type display
//      Tire icon (SVG, no emoji) · iPhone-only hamburger menu
// ─────────────────────────────────────────────────────────────────────────────
...
const AlignIcon   = p=><Ic {...p}><path d="M21 6H3M15 12H3M17 18H3"/><circle cx="20" cy="18" r="2"/><circle cx="18" cy="12" r="2"/></Ic>;
...
const ReviewsIcon = p=><Ic {...p}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/><path d="m9 10 1.5 1.5L13 8"/></Ic>;
  ...
  return (
    <button onClick={()=>onToggle(filterKey)}
      ...
      style={{flex:"1 1 0",minWidth:90,textAlign:"left",cursor:"pointer",outline:"none",
        ...
        borderRadius:T.r12,padding:"14px 14px 12px",
      ...
      <div style={{fontSize:28,fontWeight:800,lineHeight:1,color:T.textPrimary,
        marginBottom:3,letterSpacing:"-0.03em",fontVariantNumeric:"tabular-nums"}}>{value}</div>
      ...
      {alerts.map(a=>(
        <div key={a.id} style={{background:T.panelBg,border:`1px solid ${T.borderVis}`,
          ...
          <button onClick={()=>onDismiss(a.id)} style={{background:"none",border:"none",
            color:T.textMuted,cursor:"pointer",padding:2}}><XIcon size={12}/></button>
        </div>
      ))}
    </div>
  );
...
function CompleteModal({ booking, onClose, onComplete }) {
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      {[
        { variant:"with_review", icon:<StarIcon size={16} color={T.amber}/>, title:"Send with Google Review",
          desc:"Thank-you SMS with your Google review link at the bottom.", bg:T.amberBg, bd:T.amberBorder, cl:T.amber },
        { variant:"without_review", icon:<MsgIcon size={16} color={T.teal}/>, title:"Send Thank-You Only",
          desc:"Short thank-you SMS, no review link.", bg:T.tealBg, bd:T.tealBorder, cl:T.teal },
        { variant:"none", icon:<XIcon size={16} color={T.textMuted}/>, title:"Complete — No SMS",
          desc:"Mark as completed without sending any message.", bg:T.elevated, bd:T.border, cl:T.textMuted },
      ].map(({variant,icon,title,desc,bg,bd,cl})=>(
        <div key={variant} style={{background:T.elevated,border:`1.5px solid ${bd}`,
          borderRadius:T.r12,padding:"14px 16px",marginBottom:9}}>
          ...
          <Btn onClick={()=>go(variant)} disabled={busy}
            style={{width:"100%",justifyContent:"center",color:cl,
  ...
  return (
    <Overlay onClose={onClose}>
      <div style={{textAlign:"center",padding:"6px 0 2px"}}>
        <div style={{width:50,height:50,borderRadius:"50%",background:T.redBg,
          border:`1.5px solid ${T.redBorder}`,display:"flex",alignItems:"center",
          justifyContent:"center",margin:"0 auto 14px"}}><TrashIcon size={21} color={T.red}/></div>
    ...
    <Overlay onClose={onClose}>
      ...
      <div style={{display:"flex",gap:10,marginTop:16}}>
        <Btn onClick={async()=>{setBusy(true);await onSave(booking._id,{notes:note});setBusy(false);onClose();}}
          disabled={busy} icon={<CheckIcon size={15} color="#fff"/>}>
...
function SMSModal({ booking, onClose, onSend }) {
  ...
  const TYPES = [
    ...
    {k:"waitlist",          label:"Waitlist",               desc:"A spot opened up"},
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <button onClick={async()=>{setBusy(true);await onSend(booking,type);setBusy(false);}} disabled={busy}
        style={{width:"100%",padding:"12px",borderRadius:T.r8,
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:13}}>
        ...
        <div><FLbl>Date</FLbl>
          <Inp value={form.date} onChange={e=>set("date",e.target.value)} placeholder="YYYY-MM-DD"/>
        </div>
        <div><FLbl>Time</FLbl>
        ...
        <div><FLbl>Tire Size</FLbl>
          ...
          <label style={{display:"flex",alignItems:"center",gap:8,marginTop:8,
            cursor:"pointer",fontSize:13,color:form.doesntKnowTireSize?T.amber:T.textMuted}}>
        ...
        <div><FLbl>Notes</FLbl>
          <textarea value={form.notes} onChange={e=>set("notes",e.target.value)} rows={3}
            style={{width:"100%",background:T.pageBg,border:`1.5px solid ${T.border}`,
              borderRadius:T.r8,padding:"10px 13px",color:T.textPrimary,fontSize:14,
              fontFamily:T.font,resize:"vertical",outline:"none",boxSizing:"border-box"}}/>
        </div>
      </div>
    ...
    <Overlay onClose={onClose}>
      ...
      <div style={{marginTop:12}}><FLbl>Notes</FLbl>
        <textarea value={f.notes} onChange={e=>set("notes",e.target.value)} rows={2}
          style={{width:"100%",background:T.pageBg,border:`1.5px solid ${T.border}`,borderRadius:T.r8,padding:"10px 13px",color:T.textPrimary,fontSize:13,fontFamily:T.font,resize:"vertical",outline:"none",boxSizing:"border-box"}}/>
      </div>
      <div style={{display:"flex",gap:10,marginTop:16}}>
        <Btn onClick={async()=>{
  ...
  return (
    <Overlay onClose={onClose} wide>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        ...
        <Btn onClick={()=>fetchCustomers(search)} small icon={<SearchIcon size={14} color="#fff"/>}>Search</Btn>
      </div>
      {loading
        ? <div style={{textAlign:"center",padding:"20px 0",color:T.textMuted}}>Loading...</div>
        : customers.length===0
        ? <div style={{textAlign:"center",padding:"20px 0",color:T.textMuted,fontSize:13}}>No customers found.</div>
        : <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:420,overflowY:"auto"}}>
            {customers.map(c=>(
              <button key={c.phone} onClick={()=>setSelected(c)}
                ...
                <div style={{width:36,height:36,borderRadius:"50%",background:T.cardBg,
                  border:`1px solid ${T.border}`,display:"flex",alignItems:"center",
                  justifyContent:"center",flexShrink:0,fontWeight:700,fontSize:13,color:T.blueBright}}>
...
function LiveAtBay({ onSnooze, onComplete }) {
  ...
  const fetchBay = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/live-bay`,
        {headers:{"Authorization":`Bearer ${getToken()}`}});
      const d = await res.json();
  ...
  useEffect(()=>{
    fetchBay();
    const iv = setInterval(fetchBay, 60_000);
  ...
  return (
    <div style={{marginBottom:28}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
        ...
        <span style={{fontSize:11,color:T.textMuted,background:T.elevated,
          padding:"2px 8px",borderRadius:20,border:`1px solid ${T.border}`}}>
          {data?.active?.length||0} active
        </span>
        <button onClick={fetchBay} style={{background:"none",border:"none",color:T.textMuted,cursor:"pointer",padding:2,display:"flex",marginLeft:"auto"}}>
          <RefreshIcon size={14}/>
        </button>
      </div>
      ...
      {(!data?.active||data.active.length===0) ? (
        <div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r12,
          padding:"24px 16px",textAlign:"center",color:T.textMuted,fontSize:13}}>
          No customers currently in a bay.
        </div>
      ) : (
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12}}>
          {data.active.map((b,i) => {
            const bayKey = b.capacityType==="alignment" ? "alignment" : (b.assignedBay||i+1);
            const bc = BAY_COLORS[bayKey] || BAY_COLORS[1];
            const isOverdue = b.minutesRemaining<=0 && !snoozed(b);
            const needsCheck = b.minutesRemaining<=5 && b.minutesRemaining>0 && !snoozed(b);

            return (
              <div key={b._id} style={{background:bc.bg,border:`1.5px solid ${isOverdue?T.red:bc.bd}`,
                borderRadius:T.r12,padding:"14px 16px",position:"relative",
                boxShadow:isOverdue?`0 0 12px ${T.red}40`:T.shadow}}>
                {/* Bay label */}
                <div style={{position:"absolute",top:10,right:12,fontSize:10,fontWeight:700,
                  ...
                  {b.capacityType==="alignment"?"Alignment Lane":`Bay ${bayKey}`}
                </div>

                <div style={{fontSize:15,fontWeight:700,color:T.textPrimary,marginBottom:3,paddingRight:80}}>
                  {b.firstName} {b.lastName}
                </div>
                <div style={{fontSize:12,fontWeight:600,color:bc.cl,textTransform:"uppercase",
                  letterSpacing:"0.04em",marginBottom:4}}>{displaySvc(b)}</div>
                ...
                {(isOverdue||needsCheck)&&(
                  <div style={{background:T.redBg,border:`1px solid ${T.redBorder}`,
                    borderRadius:T.r8,padding:"8px 10px",marginBottom:10,fontSize:12,color:T.redText}}>
                    Is {b.firstName} done?
                  </div>
                )}
      ...
      {data?.upcoming?.length>0&&(
        <div style={{marginTop:14,padding:"10px 14px",background:T.cardBg,
          border:`1px solid ${T.border}`,borderRadius:T.r10}}>
          <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",
            color:T.textMuted,marginBottom:8}}>Up Next</div>
...
function GoogleReviews() {
  ...
  useEffect(()=>{ fetch_(); const iv=setInterval(fetch_,5*60_000); return()=>clearInterval(iv); },[fetch_]);

  const StarRating = ({rating}) => (
    <div style={{display:"flex",gap:2}}>
      {[1,2,3,4,5].map(i=>(
        <svg key={i} width="13" height="13" viewBox="0 0 24 24" fill={i<=rating?"#F59E0B":"none"}
    ...
    <div style={{marginBottom:28}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <ReviewsIcon size={16} color={T.amber}/>
          <span style={{fontSize:14,fontWeight:700,color:T.textPrimary}}>Recent Google Reviews</span>
          {data&&<span style={{fontSize:11,color:T.textMuted,background:T.elevated,
            padding:"2px 8px",borderRadius:20,border:`1px solid ${T.border}`}}>
            {data.overallRating} avg · {data.totalRatings} total
          </span>}
          {data&&!data.apiConnected&&<span style={{fontSize:10,color:T.textMuted,
            background:T.amberBg,border:`1px solid ${T.amberBorder}`,
            padding:"2px 7px",borderRadius:20}}>Demo mode</span>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={fetch_} style={{background:"none",border:"none",color:T.textMuted,cursor:"pointer",padding:2,display:"flex"}}><RefreshIcon size={14}/></button>
          <button onClick={()=>setExpanded(e=>!e)} style={{background:"none",border:"none",color:T.textMuted,cursor:"pointer",fontSize:11,fontFamily:T.font}}>
            {expanded?"Collapse":"Expand"}
          </button>
        </div>
      </div>

      {data&&!data.apiConnected&&(
        <div style={{background:T.amberBg,border:`1px solid ${T.amberBorder}`,borderRadius:T.r8,
          padding:"10px 13px",fontSize:12,color:T.amberText,marginBottom:12,lineHeight:1.6}}>
          {data.setupNote}
        </div>
      )}
      ...
      {expanded&&(
        loading
          ? <div style={{textAlign:"center",padding:"20px 0",color:T.textMuted,fontSize:13}}>Loading reviews...</div>
          : !data?.reviews?.length
          ? <div style={{textAlign:"center",padding:"20px 0",color:T.textMuted,fontSize:13}}>No reviews yet.</div>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
                    <div style={{display:"flex",alignItems:"center",gap:9}}>
                      <div style={{width:32,height:32,borderRadius:"50%",background:T.elevated,
                        border:`1px solid ${T.border}`,display:"flex",alignItems:"center",
                        justifyContent:"center",fontSize:12,fontWeight:700,color:T.blueBright,flexShrink:0}}>
                        {r.author?.[0]||"?"}
                      </div>
                      <div>
                        <div style={{fontSize:13,fontWeight:600,color:T.textPrimary}}>{r.author}</div>
                        <StarRating rating={r.rating}/>
                      </div>
                    </div>
                    <div style={{fontSize:11,color:T.textMuted}}>{r.relativeTime}</div>
                  </div>
                  {r.text&&<div style={{fontSize:13,color:T.textSecond,lineHeight:1.6,
                    paddingLeft:41}}>{r.text.length>200?r.text.slice(0,200)+"…":r.text}</div>}
                  {!r.real&&<div style={{fontSize:10,color:T.textMuted,marginTop:4,paddingLeft:41}}>Sample review</div>}
                </div>
              ))}
            </div>
      )}
...
function ApptCard({ booking, onUpdate, onDelete, onEdit, onSMS, onNote, onComplete, visitCount, isCompleted=false }) {
  ...
  const capBadge = booking.capacityType==="alignment"
    ? <span style={{fontSize:10,fontWeight:600,padding:"1px 6px",borderRadius:20,
        background:T.purpleBg,color:T.purpleText,border:`1px solid ${T.purpleBorder}`,
        whiteSpace:"nowrap"}}>Alignment lane</span>
    : booking.capacityType==="bay"
    ? <span style={{fontSize:10,fontWeight:600,padding:"1px 6px",borderRadius:20,
        background:T.blueSubtle,color:T.blueBright,border:`1px solid rgba(37,99,235,.3)`,
        whiteSpace:"nowrap"}}>Bay</span>
    : null;
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{background:hov?T.cardBgHov:T.cardBg,
        ...
        transition:"background .14s,border-color .14s",opacity:busy?0.7:1}}>

      <div style={{padding:"16px 18px",display:"flex",flexWrap:"wrap",alignItems:"center",gap:13}}>
        {/* Time */}
        <div style={{flexShrink:0,minWidth:68,padding:"9px 11px",
        ...
        <div style={{flex:1,minWidth:150}}>
          ...
          {booking.notes&&(
            <div style={{fontSize:11,color:T.orange,fontStyle:"italic",marginTop:4,
  ...
  return (
    <div style={{marginBottom:24}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
        marginBottom:9,padding:"7px 13px",background:T.elevated,borderRadius:T.r8,border:`1px solid ${T.border}`}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <CalIcon size={13} color={T.blue}/>
          <span style={{fontSize:13,fontWeight:700,color:T.textPrimary}}>{fmtDate(date)}</span>
          <span style={{fontSize:10,color:T.textMuted,background:T.cardBg,padding:"1px 7px",borderRadius:20,border:`1px solid ${T.border}`}}>
...
const printDay = (bookings, date) => {
  ...
  <table><tr><th>Time</th><th>Customer</th><th>Phone</th><th>Service</th><th>Tire Size</th><th>Status</th><th>Notes</th></tr>
  ${rows.map(b=>`<tr><td><strong>${b.time}</strong><br/><span style="color:#888">${b.duration}min</span></td>
  <td><strong>${b.firstName} ${b.lastName}</strong></td><td>${b.phone}</td>
...
export default function RoadstarDashboard({ onLogout }) {
  ...
  const playSound = useCallback(()=>{
    try {
      ...
      const tone=(f,s,d)=>{
        ...
        o.connect(g);g.connect(ac.destination);o.type="sine";o.frequency.setValueAtTime(f,s);
        g.gain.setValueAtTime(0,s);g.gain.linearRampToValueAtTime(0.4,s+0.025);
  ...
  const load = useCallback(async(silent=false)=>{
    ...
    try {
      ...
      setConnected(false);
      if(!silent) pushAlert("Could not load bookings","error");
  ...
  // FIXED: complete with correct variant mapping
  const handleComplete = async(booking, variant)=>{
    try {
      ...
      const msg = variant==="with_review" ? "Marked complete + review SMS sent"
        : variant==="without_review" ? "Marked complete + thank-you SMS sent"
        : "Marked complete";
      pushAlert(msg);
  ...
  const handleBaySnooze = async(id)=>{
    try {
      await fetch(`${API_BASE}/bookings/${id}/bay-snooze`,{
  ...
  // Filter toggle (fixed — any re-click clears)
  ...
  // ── Toolbar items (shared between header and mobile menu) ─────────────────────
  const toolbarItems = [
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* ── Responsive: iPhone-only hamburger (< 640px) ─────────────────── */
        /* iPads are typically 768px+, iPhones are < 640px */
      ...
      {mobileMenu&&(
        <div onClick={()=>setMobileMenu(false)}
          style={{position:"fixed",inset:0,background:"rgba(2,4,12,.7)",zIndex:500,backdropFilter:"blur(4px)"}}/>
      )}
      {/* Mobile slide-down menu */}
      {mobileMenu&&(
        <div style={{position:"fixed",top:57,right:12,zIndex:600,
          background:T.panelBg,border:`1px solid ${T.borderVis}`,
          borderRadius:T.r12,padding:"10px 6px",boxShadow:T.shadowLg,minWidth:180}}>
      ...
      <header style={{background:T.pageBg,borderBottom:`1px solid ${T.border}`,
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div className="toolbar-desktop">
            {toolbarItems.map(item=>(
              <Btn key={item.label} small onClick={item.onClick}
                icon={item.icon}
                variant="ghost"
          ...
          {/* iPhone hamburger */}
          <button className="hamburger-btn" onClick={()=>setMobileMenu(m=>!m)}>
            <MenuIcon size={18}/>
          </button>
        </div>
      </header>
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"22px 14px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
          <div style={{display:"flex",gap:2,background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r10,padding:4}}>
            {[{id:"queue",label:"Live Queue",icon:<CalIcon size={13}/>},{id:"completed",label:"Completed",icon:<FlagIcon size={13}/>}]
              .map(t=>{
                  ...
                  <button key={t.id} onClick={()=>setTab(t.id)}
                    ...
                    {t.id==="completed"&&counts.completed>0&&(
                      <span style={{background:T.tealBg,color:T.teal,border:`1px solid ${T.tealBorder}`,
                        padding:"1px 6px",borderRadius:20,fontSize:10,fontWeight:700}}>{counts.completed}</span>
                    )}
                  </button>
                );
              })}
            ...
            <div style={{position:"relative",width:240}}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)}
                ...
                style={{width:"100%",background:T.cardBg,border:`1px solid ${T.border}`,
                  borderRadius:T.r8,padding:"8px 12px 8px 32px",color:T.textPrimary,
        ...
        {filter!=="all"&&(
          <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:13}}>
            ...
            <span style={{background:sm(filter==="today"?"confirmed":filter).bg,
              ...
              letterSpacing:"0.05em",textTransform:"uppercase"}}>{filter}</span>
          ...
          loading
            ? <div style={{display:"flex",flexDirection:"column",gap:9}}>
                {[1,2,3].map(i=><div key={i} style={{background:T.cardBg,border:`1px solid ${T.border}`,
                  borderRadius:T.r12,height:88,animation:"shimmer 1.6s ease-in-out infinite"}}/>)}
              </div>
            : Object.keys(byDate).length===0
            ? <div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r12,
                padding:"44px 20px",textAlign:"center",color:T.textMuted,fontSize:13}}>
        ...
        {tab==="completed"&&(
          <div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"8px 13px",background:T.tealBg,border:`1px solid ${T.tealBorder}`,
            ...
            {completedBookings.length===0
              ? <div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r12,
                  padding:"32px 20px",textAlign:"center",color:T.textMuted,fontSize:13}}>
        ...
        {/* LIVE AT BAY — shown below both tabs */}
