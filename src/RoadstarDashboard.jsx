// ─────────────────────────────────────────────────────────────────────────────
// RoadstarDashboard.jsx  v7.3
//
// Changes:
//   - Recently Deleted tab — shows soft-deleted bookings from last 15 days
//   - Restore button on deleted items
//   - Delete now soft-deletes (server does the actual soft delete)
//   - No Google Reviews section
//   - Footer: Built by Social Aura
//   - Mobile drawer (iPhone only < 640px)
//   - Complete button works on pending/confirmed/waitlist
//   - Correct service durations via SVC_DEFS fallback
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Service duration lookup (mirrors server SERVICE_DEFS) ─────────────────────
const SVC_DEFS = {
...
// Use _resolvedDuration from server when available, otherwise look up from SVC_DEFS.
// Falls back to stored value only if stored > 10 (correctly saved).
const effectiveOcc = b => {
  ...
  const dur = (stored && stored > 10) ? stored : (def?.service_duration || 10);
...
const TireIcon    = p=><Ic {...p}><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3.5"/><path d="M12 2.5v4M12 17.5v4M2.5 12h4M17.5 12h4"/><path d="M5.5 5.5l2.8 2.8M15.7 15.7l2.8 2.8M18.5 5.5l-2.8 2.8M8.3 15.7l-2.8 2.8"/></Ic>;
...
function Btn({ onClick, children, variant="primary", icon=null, small=false, disabled=false, style={} }) {
  const [h,setH] = useState(false);
  const S = {
    primary: {bg:h?T.blueLight:T.blue,       bd:T.blue,          cl:"#fff"},
    ...
    restore: {bg:h?T.greenBg:T.elevated,     bd:h?T.greenBorder:T.border,cl:h?T.green:T.textSecond},
...
function CompleteModal({ booking, onClose, onComplete }) {
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <HDivider/>
      <p style={{fontSize:13,color:T.textSecond,lineHeight:1.6,marginBottom:16}}>Send a thank-you SMS to the customer?</p>
      {[
        {variant:"with_review",    icon:<StarIcon size={16} color={T.amber}/>,     title:"Send with Google Review",  desc:"Thank-you + Google review link at bottom.", bg:T.amberBg, bd:T.amberBorder, cl:T.amber},
        {variant:"without_review", icon:<MsgIcon  size={16} color={T.teal}/>,      title:"Send Thank-You Only",       desc:"Thank-you SMS, no review link.",           bg:T.tealBg,  bd:T.tealBorder,  cl:T.teal},
        {variant:"none",           icon:<XIcon    size={16} color={T.textMuted}/>, title:"Complete — No SMS",         desc:"Mark complete without any message.",        bg:T.elevated,bd:T.border,       cl:T.textMuted},
  ...
  return (
    <Overlay onClose={onClose}>
      <div style={{textAlign:"center",padding:"6px 0 2px"}}>
        ...
        <div style={{fontSize:17,fontWeight:700,color:T.textPrimary,marginBottom:7}}>Move to Recently Deleted?</div>
        <div style={{fontSize:13,color:T.textSecond,lineHeight:1.65,marginBottom:6}}>
        ...
        <div style={{fontSize:12,color:T.textMuted,marginBottom:18}}>
          This booking will be moved to Recently Deleted and permanently removed after 15 days. You can restore it any time before then.
        </div>
        <div style={{display:"flex",gap:10,justifyContent:"center"}}>
          <Btn onClick={async()=>{setBusy(true);await onConfirm(booking._id);setBusy(false);}}
            ...
            {busy?"Moving…":"Yes, Delete"}
          </Btn>
          <Btn onClick={onClose} disabled={busy} variant="ghost">Cancel</Btn>
        </div>
      </div>
    </Overlay>
  );
...
function SMSModal({ booking, onClose, onSend }) {
  ...
  return (
    <Overlay onClose={onClose}>
      <MHdr icon={<MsgIcon size={18}/>} title="Send SMS" sub={`${booking.firstName} ${booking.lastName} · ${booking.phone}`}/>
      <HDivider/>
      <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:16}}>
        {TYPES.map(({k,label,desc})=>{
          const active=type===k;
          return (
            <button key={k} onClick={()=>setType(k)} style={{display:"flex",alignItems:"center",gap:11,
              padding:"10px 13px",borderRadius:T.r10,cursor:"pointer",textAlign:"left",
              background:active?T.blueSubtle:T.elevated,border:`1.5px solid ${active?T.blue:T.border}`,transition:"all .12s"}}>
              <div style={{width:15,height:15,borderRadius:"50%",flexShrink:0,border:`2px solid ${active?T.blue:T.textMuted}`,
                background:active?T.blue:"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:13}}>
        ...
        <div><FLbl>Time</FLbl><Sel value={f.time} onChange={e=>set("time",e.target.value)} options={TIME_SLOTS.map(t=>({value:t,label:t}))}/></div>
      </div>
...
function CustomerModal({ onClose }) {
  ...
  const fetchC = useCallback(async(q="")=>{
    setLoading(true);
    try{const res=await fetch(`${API_BASE}/customers?search=${encodeURIComponent(q)}`,{headers:{"Authorization":`Bearer ${getToken()}`}});
        ...
        :<div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:420,overflowY:"auto"}}>
          {customers.map(c=>(
            <button key={c.phone} onClick={()=>setSelected(c)}
              ...
              <div style={{fontSize:11,color:T.textMuted,textAlign:"right"}}>{c.visitCount} visit{c.visitCount!==1?"s":""}</div>
            </button>
          ))}
        </div>
      }
        ...
        {TIME_SLOTS.map(s=>{
          ...
          return (
            <div key={s} style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{fontSize:11.5,fontWeight:600,color:T.textSecond,width:68,textAlign:"right",flexShrink:0}}>{s}</div>
...
function LiveAtBay({ onSnooze, onComplete }) {
  ...
  const fetchBay = useCallback(async()=>{
    try{const res=await fetch(`${API_BASE}/live-bay`,{headers:{"Authorization":`Bearer ${getToken()}`}});
      const d=await res.json();if(d.success)setData(d);}
    catch(e){}finally{setLoading(false);}
  },[]);
  ...
  return (
    <div style={{marginBottom:28}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
        ...
        <span style={{fontSize:11,color:T.textMuted,background:T.elevated,padding:"2px 8px",borderRadius:20,border:`1px solid ${T.border}`}}>{data?.active?.length||0} active</span>
      ...
      {(!data?.active||data.active.length===0)
        ? <div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r12,padding:"22px 16px",textAlign:"center",color:T.textMuted,fontSize:13}}>No customers currently in a bay.</div>
            ...
            {data.active.map((b,i)=>{
              ...
              return (
                <div key={b._id} style={{background:bc.bg,border:`1.5px solid ${isOver?T.red:bc.bd}`,
                  ...
                  <div style={{position:"absolute",top:10,right:12,fontSize:10,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",
                    color:bc.cl,background:T.cardBg,padding:"2px 7px",borderRadius:20,border:`1px solid ${bc.bd}`}}>
                    {b.resourcePool==="alignment"?"Alignment":bayKey===1?"Bay 1":bayKey===2?"Bay 2":"Bay 3"}
                  </div>
                  <div style={{fontSize:15,fontWeight:700,color:T.textPrimary,marginBottom:3,paddingRight:80}}>{b.firstName} {b.lastName}</div>
                  ...
                  {(isOver||needsChk)&&(
                    <div style={{background:T.redBg,border:`1px solid ${T.redBorder}`,borderRadius:T.r8,
                      padding:"9px 12px",marginBottom:10,fontSize:13,fontWeight:600,color:T.redText,display:"flex",alignItems:"center",gap:8}}>
                      ⏰ Is this done? — {b.firstName} {b.lastName}
                    </div>
                  )}
                  <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                    <Btn small onClick={()=>onComplete(b)} variant="teal" icon={<CheckIcon size={12} color={T.teal}/>}>Done</Btn>
                    {(isOver||needsChk)&&<Btn small onClick={()=>onSnooze(b._id)} variant="amber" icon={<ClockIcon size={12} color={T.amber}/>}>+10 min</Btn>}
                  </div>
                </div>
              );
            })}
    ...
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{padding:"16px 18px",display:"flex",flexWrap:"wrap",alignItems:"center",gap:13}}>
        ...
        <div style={{flex:1,minWidth:150}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:3,flexWrap:"wrap"}}>
            <div style={{fontSize:12,fontWeight:700,color:s.color,letterSpacing:"0.03em",textTransform:"uppercase"}}>{displaySvc(booking)}</div>
            {poolBadge}
          </div>
          ...
          {booking.notes&&(
            <div style={{fontSize:11,color:T.orange,fontStyle:"italic",marginTop:4,display:"flex",alignItems:"flex-start",gap:5}}>
              <NoteIcon size={11} color={T.orange} style={{marginTop:1,flexShrink:0}}/>{booking.notes}
            </div>
          )}
        </div>
        ...
        <div style={{borderTop:`1px solid ${T.border}`,background:T.elevated,padding:"12px 18px",display:"flex",gap:10,alignItems:"flex-end",flexWrap:"wrap"}}>
          <div style={{minWidth:180}}>
            ...
            <Sel value={newTime} onChange={e=>setNewTime(e.target.value)}
              options={["Select…",...TIME_SLOTS].map(t=>({value:t,label:t}))} style={{fontSize:13,padding:"9px 12px"}}/>
          </div>
...
function RecentlyDeleted({ onRestore }) {
  ...
  const load = useCallback(async()=>{
    setLoading(true);
    try{
      const res = await fetch(`${API_BASE}/recently-deleted`,{headers:{"Authorization":`Bearer ${getToken()}`}});
      const d   = await res.json();
      setItems(d.bookings||[]);
    }catch(e){}finally{setLoading(false);}
  },[]);
  ...
  const handleRestore = async(id)=>{
    ...
    try{
      const res  = await fetch(`${API_BASE}/bookings/${id}/restore`,{method:"PATCH",headers:{"Authorization":`Bearer ${getToken()}`,"Content-Type":"application/json"}});
      const data = await res.json();
      if(data.success){
        setItems(prev=>prev.filter(b=>b._id!==id));
        onRestore();
      }
    }catch(e){}finally{setRestoring(r=>({...r,[id]:false}));}
  };

  const daysLeft = dt => {
    if(!dt) return 0;
    const diff = 15 - Math.floor((Date.now()-new Date(dt))/(1000*60*60*24));
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
        padding:"8px 13px",background:T.redBg,border:`1px solid ${T.redBorder}`,
        borderRadius:T.r8,marginBottom:11}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <TrashIcon size={13} color={T.red}/>
          <span style={{fontSize:13,fontWeight:700,color:T.red}}>Recently Deleted — {items.length}</span>
        </div>
        <button onClick={load} style={{background:"none",border:"none",color:T.textMuted,cursor:"pointer",display:"flex"}}><RefreshIcon size={14}/></button>
      </div>

      {loading
        ? <div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r12,padding:"28px 20px",textAlign:"center",color:T.textMuted,fontSize:13}}>Loading...</div>
        : items.length===0
        ? <div style={{background:T.cardBg,border:`1px solid ${T.border}`,borderRadius:T.r12,padding:"32px 20px",textAlign:"center"}}>
            <div style={{width:44,height:44,borderRadius:"50%",background:T.elevated,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px"}}><TrashIcon size={18} color={T.textMuted}/></div>
            <div style={{fontSize:13,color:T.textMuted}}>No recently deleted bookings.</div>
            <div style={{fontSize:12,color:T.textMuted,marginTop:4}}>Deleted bookings appear here for 15 days before permanent removal.</div>
          </div>
        : <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {items.map(b=>{
              const days = daysLeft(b.deletedAt);
              const urgent = days<=3;
              return (
                <div key={b._id} style={{background:T.cardBg,border:`1px solid ${T.redBorder}`,
                  borderLeft:`4px solid ${urgent?T.red:T.textMuted}`,borderRadius:T.r12,
                  padding:"14px 18px",display:"flex",flexWrap:"wrap",alignItems:"center",gap:13,
                  opacity:0.85}}>
                  {/* Time block */}
                  <div style={{flexShrink:0,minWidth:68,padding:"9px 11px",background:T.elevated,border:`1px solid ${T.border}`,borderRadius:T.r10,textAlign:"center"}}>
                    <div style={{fontSize:14,fontWeight:800,color:T.textMuted,lineHeight:1,marginBottom:3}}>{b.time}</div>
                    <div style={{fontSize:10,color:T.textMuted}}>{b.date}</div>
                  </div>
                  {/* Info */}
                  <div style={{flex:1,minWidth:150}}>
                    <div style={{fontSize:14,fontWeight:700,color:T.textSecond,marginBottom:3}}>{b.firstName} {b.lastName}</div>
                    <div style={{fontSize:12,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:"0.04em",marginBottom:3}}>{displaySvc(b)}</div>
                    <div style={{fontSize:11,color:T.textMuted}}>{b.phone}</div>
                    {b.deletedAt&&(
                      <div style={{fontSize:10,color:T.textMuted,marginTop:4}}>
                        Deleted {fmtDeletedAt(b.deletedAt)}
                        <span style={{marginLeft:8,color:urgent?T.red:T.textMuted,fontWeight:urgent?600:400}}>
                          {days===0?"Permanent deletion today":`${days} day${days!==1?"s":""} until permanent removal`}
                        </span>
                      </div>
                    )}
                  </div>
                  {/* Restore button */}
                  <Btn onClick={()=>handleRestore(b._id)}
      ...
      <div style={{fontSize:11,color:T.textMuted,marginTop:10,textAlign:"center",lineHeight:1.6}}>
        Deleted bookings are permanently removed after 15 days. Restore them any time before that.
      </div>
    </div>
  );
}
...
export default function RoadstarDashboard({ onLogout }) {
  ...
  const unlockAudio = useCallback(()=>{
    ...
    try{audioCtxRef.current=new(window.AudioContext||window.webkitAudioContext)();
      if(audioCtxRef.current.state==="suspended")audioCtxRef.current.resume().catch(()=>{});}catch(e){}
  },[]);
  useEffect(()=>{
  ...
  const playSound = useCallback(()=>{
    try{const ac=audioCtxRef.current||new(window.AudioContext||window.webkitAudioContext)();
      ...
      tone(800,now,.35);tone(1050,now+.22,.45);}catch(e){}
  },[]);
  ...
  const handleDelete = async(id)=>{
    try{
      await deleteBooking(id); // soft delete — marks deleted=true on server
      setBookings(b=>b.filter(x=>x._id!==id));
      prevCount.current--;
      pushAlert("Booking moved to Recently Deleted. You can restore it within 15 days.");
      setDeleteB(null);
  ...
  const handleComplete = async(booking,variant)=>{
    try{
      const updated=await updateBooking(booking._id,{status:"completed",completedSmsVariant:variant,sendSMS:variant!=="none"});
      setBookings(b=>b.map(x=>x._id===booking._id?updated:x));
      pushAlert(variant==="with_review"?"Marked complete + review SMS sent":variant==="without_review"?"Marked complete + thank-you SMS sent":"Marked complete");
    }catch(e){pushAlert(e.message||"Complete failed","error");}
    setCompleteB(null);
  };
  const handleBaySnooze = async(id)=>{
    try{await fetch(`${API_BASE}/bookings/${id}/bay-snooze`,{method:"PATCH",headers:{"Authorization":`Bearer ${getToken()}`}});pushAlert("Check snoozed 10 min");}
    catch(e){}
  };
  ...
  return (
    <>
      ...
      {mobileMenu&&<div onClick={()=>setMobileMenu(false)} style={{position:"fixed",inset:0,background:"rgba(2,4,12,.75)",zIndex:500,backdropFilter:"blur(6px)"}}/>}
      {/* Mobile left drawer */}
      <div style={{position:"fixed",top:0,left:0,bottom:0,zIndex:600,width:280,
        ...
        <div style={{padding:"16px 18px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:30,height:30,borderRadius:"50%",background:T.cardBg,border:`1px solid ${T.borderVis}`,display:"flex",alignItems:"center",justifyContent:"center"}}><TireIcon size={16} color={T.blue}/></div>
          ...
          <button onClick={()=>setMobileMenu(false)} style={{background:T.elevated,border:`1px solid ${T.border}`,borderRadius:T.r8,width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",color:T.textMuted,cursor:"pointer"}}><XIcon size={13}/></button>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"8px 8px"}}>
          {toolbarItems.map(item=>(
            <button key={item.label} onClick={item.onClick}
              style={{display:"flex",alignItems:"center",gap:12,width:"100%",padding:"13px 14px",background:"none",border:"none",borderRadius:T.r10,cursor:"pointer",color:item.danger?T.red:T.textPrimary,fontSize:15,fontWeight:500,fontFamily:T.font,textAlign:"left",transition:"background .12s",marginBottom:2}}
              onMouseEnter={e=>e.currentTarget.style.background=T.elevated}
              onMouseLeave={e=>e.currentTarget.style.background="none"}>
              <div style={{width:36,height:36,borderRadius:T.r10,flexShrink:0,background:item.danger?T.redBg:T.elevated,border:`1px solid ${item.danger?T.redBorder:T.border}`,display:"flex",alignItems:"center",justifyContent:"center"}}>{item.icon}</div>
              {item.label}
            </button>
          ))}
        </div>
        <div style={{padding:"14px 18px",borderTop:`1px solid ${T.border}`,textAlign:"center"}}>
          <span style={{fontSize:11,color:T.textMuted}}>Built by{" "}
            <a href="https://socialaura.ca" target="_blank" rel="noopener noreferrer" style={{color:T.textSecond,textDecoration:"none",fontWeight:600}}>Social Aura</a>
          </span>
        </div>
      </div>
      ...
      <header style={{background:T.pageBg,borderBottom:`1px solid ${T.border}`,padding:"10px 18px",
        display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,gap:10}}>
        <Logo/>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"22px 14px"}}>
            ...
            {[
              ...
              return (
                <button key={t.id} onClick={()=>setTab(t.id)}
                  style={{display:"flex",alignItems:"center",gap:7,padding:"8px 13px",borderRadius:T.r8,
                    border:"none",fontWeight:600,fontSize:13,fontFamily:T.font,cursor:"pointer",
                    transition:"all .13s",background:active?T.elevated:"transparent",
                    color:active?(t.id==="deleted"?T.red:T.textPrimary):T.textMuted}}>
                  <span style={{color:active?(t.id==="deleted"?T.red:T.blue):T.textMuted}}>{t.icon}</span>
                  {t.label}
        ...
        {tab==="completed"&&(
          <div>
            <div style={{display:"flex",alignItems:"center",padding:"8px 13px",background:T.tealBg,border:`1px solid ${T.tealBorder}`,borderRadius:T.r8,marginBottom:11}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
